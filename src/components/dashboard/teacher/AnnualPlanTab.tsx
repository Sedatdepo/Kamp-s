...
function GuidanceAnnualPlan({ currentClass }: { currentClass: Class | null }) {
  ...
  const [termReportGorusmeText, setTermReportGorusmeText] = useState("");
  const [termReportYonlendirilenText, setTermReportYonlendirilenText] = useState("");
  const [termReportVeliGorusmeText, setTermReportVeliGorusmeText] = useState("");

  const handleGenerateReportContent = async (reportType: 'term' | 'year', field: 'gorusme' | 'yonlendirilen' | 'veli') => {
    setIsGenerating(true);
    let agendaTitle = "";
    if (field === 'gorusme') agendaTitle = `${selectedGrade}. Sınıf ${reportType === 'term' ? 'Dönem' : 'Yıl'} Sonu Sınıf Genel Durumu Değerlendirmesi`;
    ...
    try {
      ...
      if (reportType === 'term') {
          if (field === 'gorusme') setTermReportGorusmeText(response.generatedText);
          if (field === 'yonlendirilen') setTermReportYonlendirilenText(response.generatedText);
          if (field === 'veli') setTermReportVeliGorusmeText(response.generatedText);
      } else {
  ...
  const downloadWord = (content, filename) => {
    // Word indirme fonksiyonu (detaylar önceki kod bloğunda mevcut, burada kısaltıldı)
  ...
  const exportAnnualPlan = () => {
    // Yıllık Plan dışa aktarma (detaylar önceki kod bloğunda mevcut)
  };

  const exportActivityReport = () => {
    const currentActivities = plans[selectedGrade];
    const activity = currentActivities[selectedActivityIndex];
    // Etkinlik Raporu dışa aktarma (detaylar önceki kod bloğunda mevcut)
  };
  
  const exportTermReport = () => {
      // Dönem Sonu Raporu Dışa Aktarma
  };

  const exportEndYearReport = () => {
      // Yıl Sonu Raporu Dışa Aktarma
  };

  const renderAnnualPlan = () => {
    const currentActivities = plans[selectedGrade];
    return (
        // JSX for rendering the annual plan table
        <div>Yıllık Plan Tablosu Burada Olacak</div>
    )
  };

  const renderActivityReport = () => {
     // JSX for rendering the activity report
     return <div>Etkinlik Raporu Formu Burada Olacak</div>
  };
  
  const renderTermReport = () => {
     // JSX for rendering term report
      return (
         <Card className="animate-in fade-in">
              <CardHeader>
                  <CardTitle>1. Dönem Sonu Sınıf Rehberlik Çalışmaları Raporu</CardTitle>
                  <CardDescription>Bu rapor, ŞÖK toplantısında sunulmak üzere hazırlanmıştır.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                  <div className="space-y-2">
                      <Label>1. Sınıfın Genel Durumu (Akademik, Sosyal vb.)</Label>
                      <Textarea id="term_gorusme" rows={6} value={termReportGorusmeText} onChange={(e) => setTermReportGorusmeText(e.target.value)} />
                      <Button onClick={() => handleGenerateReportContent('term', 'gorusme')} disabled={isGenerating}>
                        {isGenerating ? <Loader2 className="animate-spin mr-2"/> : <Wand2 className="mr-2"/>} AI ile Doldur
                      </Button>
                  </div>
                  <div className="space-y-2">
                      <Label>2. Rehberlik Servisine Yönlendirilen Öğrenciler ve Nedenleri</Label>
                      <Textarea id="term_yonlendirilen" rows={4} value={termReportYonlendirilenText} onChange={(e) => setTermReportYonlendirilenText(e.target.value)}/>
                       <Button onClick={() => handleGenerateReportContent('term', 'yonlendirilen')} disabled={isGenerating}>
                        {isGenerating ? <Loader2 className="animate-spin mr-2"/> : <Wand2 className="mr-2"/>} AI ile Doldur
                      </Button>
                  </div>
                  <div className="space-y-2">
                      <Label>3. Yapılan Veli Görüşmeleri ve Sonuçları</Label>
                       <Textarea id="term_veli" rows={4} value={termReportVeliGorusmeText} onChange={(e) => setTermReportVeliGorusmeText(e.target.value)}/>
                       <Button onClick={() => handleGenerateReportContent('term', 'veli')} disabled={isGenerating}>
                        {isGenerating ? <Loader2 className="animate-spin mr-2"/> : <Wand2 className="mr-2"/>} AI ile Doldur
                      </Button>
                  </div>
                   <div className="space-y-2">
                      <Label>4. Rehberlik Servisinden Beklentiler</Label>
                      <Textarea rows={3} defaultValue="Öğrencilerin sınav kaygısı ve mesleki yönelim konusunda bilgilendirilmesi." />
                  </div>
              </CardContent>
              <div className="p-6 pt-0"><Button onClick={exportTermReport}><Download className="mr-2"/> Raporu İndir</Button></div>
        </Card>
      )
  };

  const renderEndYearReport = () => {
    // JSX for rendering end of year report
     return (
        <Card className="animate-in fade-in">
              <CardHeader>
                  <CardTitle>Yıl Sonu Sınıf Rehberlik Çalışmaları Raporu</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                  <div className="space-y-2">
                      <Label>1. Yıl Boyunca Yapılan Rehberlik Çalışmalarının Özeti</Label>
                      <Textarea id="year_gorusme" rows={6} value={gorusmeText} onChange={(e) => setGorusmeText(e.target.value)}/>
                       <Button onClick={() => handleGenerateReportContent('year', 'gorusme')} disabled={isGenerating}>
                        {isGenerating ? <Loader2 className="animate-spin mr-2"/> : <Wand2 className="mr-2"/>} AI ile Doldur
                      </Button>
                  </div>
                   <div className="space-y-2">
                      <Label>2. Rehberlik Servisine Yönlendirilen Öğrenciler ve Sonuçları</Label>
                      <Textarea id="year_yonlendirilen" rows={4} value={yonlendirilenText} onChange={(e) => setYonlendirilenText(e.target.value)}/>
                       <Button onClick={() => handleGenerateReportContent('year', 'yonlendirilen')} disabled={isGenerating}>
                        {isGenerating ? <Loader2 className="animate-spin mr-2"/> : <Wand2 className="mr-2"/>} AI ile Doldur
                      </Button>
                  </div>
                  <div className="space-y-2">
                      <Label>3. Yapılan Veli Görüşmeleri</Label>
                       <Textarea id="year_veli" rows={4} value={veliGorusmeText} onChange={(e) => setVeliGorusmeText(e.target.value)}/>
                       <Button onClick={() => handleGenerateReportContent('year', 'veli')} disabled={isGenerating}>
                        {isGenerating ? <Loader2 className="animate-spin mr-2"/> : <Wand2 className="mr-2"/>} AI ile Doldur
                      </Button>
                  </div>
                   <div className="space-y-2">
                      <Label>4. Gelecek Yıla Yönelik Öneriler</Label>
                      <Textarea rows={3} defaultValue="Öğrencilerin üniversite sınavı motivasyonunu artırmaya yönelik seminerler düzenlenebilir." />
                  </div>
              </CardContent>
              <div className="p-6 pt-0"><Button onClick={exportEndYearReport}><Download className="mr-2"/> Raporu İndir</Button></div>
        </Card>
      )
  };
