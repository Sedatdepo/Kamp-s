...
import { Loader2, BookText, Clock, CalendarIcon, ArrowLeft, ClipboardList } from 'lucide-react';
...
export function PerformanceHomeworkTab({ student, classId, assignmentType, title, description }: { student: Student; classId: string; assignmentType: 'performance' | 'project'; title: string; description: string; }) {
  ...
  if (homeworksLoading) {
    return <div className="flex justify-center p-6"><Loader2 className="h-8 w-8 animate-spin" /></div>;
  }
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
          {sortedHomeworks.length > 0 ? (
            ...
            <div className="text-center py-10 bg-muted/50 rounded-lg">
              <p className="text-sm text-muted-foreground">Henüz verilmiş bir {assignmentType === 'performance' ? 'performans ödevi' : 'proje ödevi'} yok.</p>
            </div>
          )}
        </div>
      </CardContent>
