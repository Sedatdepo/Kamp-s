...
import { PlusCircle, Save, X, Trash2, Plus } from 'lucide-react';
import { Question } from '@/lib/types';

export const CreateAssignmentModal = ({ isOpen, onClose, onSave }: any) => {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[90vh]">
          ...
          <div className="bg-gray-50 px-6 py-4 border-t border-gray-200 flex justify-end gap-3 rounded-b-2xl">
            ...
            <button onClick={handleSave} className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg flex items-center gap-2 font-medium shadow-sm"><Save size={16} /> Ödevi Oluştur</button>
          </div>
        </div>
      </div>
    );
