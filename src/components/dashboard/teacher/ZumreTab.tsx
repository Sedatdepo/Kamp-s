...
export default function ZumreTab() {
  ...
  const saveUserScenario = (index: number) => {
    ...
    if (!agendaItem || !scenarioText.trim()) {
      alert("Kaydedilecek senaryo metni boş olamaz.");
      return;
    }
    setDb(prev => {
    ...
    alert("Senaryo başarıyla havuzunuza kaydedildi!");
  };
                        ...
                        <>
                            ...
                            {(userScenarios[formData.gundemMaddeleri[activeScenarioIndex].madde] || []).length > 0 ? (
                                (userScenarios[formData.gundemMaddeleri[activeScenarioIndex].madde]).map((text: string, idx: number) => (
                                    <div key={`user-${idx}`} className="p-4 border border-green-200 rounded-xl hover:bg-green-50 hover:border-green-300 cursor-pointer transition-colors" onClick={() => useScenario(text)}>
