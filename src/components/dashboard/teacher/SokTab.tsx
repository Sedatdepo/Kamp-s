...
export default function SokTab({ teacherProfile, currentClass }: { teacherProfile: TeacherProfile | null, currentClass: any }) {
    ...
    const defaultValues = useMemo<FormData>(() => ({
        ...
        katilimcilar: VARSAYILAN_BRANSLAR.map(b => ({ brans: b, adSoyad: (b === "Sınıf Rehber Öğretmeni" ? teacherProfile?.name : "") || '' })),
    ...
    const openSaveDialog = () => {
        ...
        setSaveNameInput(selectedRecordId ? archives.find(r => r.id === selectedRecordId)?.name || `${vals.academicYear} - ${vals.sinif} ŞÖK` : `${vals.academicYear} - ${vals.sinif} ŞÖK`);
    ...
    const handleSaveToArchive = () => {
        ...
        const docId = currentData.id || `sok_${Date.now()}`;
        ...
        setLocalDb(prev => {
            const existingIndex = (prev.sokDocuments || []).findIndex(r => r.id === newDoc.id);
    ...
    const generateDocumentHTML = (data: FormData) => {
        ...
        return `
          ...
          <style>body { font-family: 'Times New Roman', Times, serif; font-size: 12pt; line-height: 1.5; } .container { width: 90%; margin: auto; } table { width: 100%; border-collapse: collapse; } th, td { border: 1px solid black; padding: 5px; }</style>
              ...
              <h3 style="text-align: center;">T.C.<br/>${data.okulAdi ? tr(data.okulAdi.toLocaleUpperCase('tr-TR')) : '...................... MÜDÜRLÜĞÜ'}<br/>${tr(data.academicYear)} EĞİTİM-ÖĞRETİM YILI ${tr(data.sinif)} ŞUBESİ<br/>${tr(data.donem.toUpperCase())} ŞUBE ÖĞRETMENLER KURULU TOPLANTI TUTANAĞI</h3>
              <br/>
             ...
             <header className="sticky top-0 z-10 border-b bg-white/90 backdrop-blur-sm px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500">Şube Öğretmenler Kurulu • Otomatik Kayıt & Yapay Zeka</p>
                    </div>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    <Button onClick={openSaveDialog} variant="outline"><Save className="mr-2 h-4 w-4"/> Arşive Kaydet</Button>
                    <Button onClick={handleExport} className="bg-orange-600 hover:bg-orange-700 text-white"><FileDown className="mr-2 h-4 w-4"/> Word</Button>
                </div>
            </header>
