        ...
        if (index < lines.length) {
            ...
            if (!isNaN(grade)) {
                ...
                const gradeFieldMap: Record<GradeType, string> = {
                  ...
                  perf1: `${termGradesKey}.perf1`,
                  perf2: `${termGradesKey}.perf2`,
                };
              ...
              {sortedStudents.map((student) => {
                ...
                return (
                    <TableRow key={student.id}>
                        ...
                        <TableCell>{termGrades?.perf1 ?? 'Girmedi'}</TableCell>
                        <TableCell>{termGrades?.perf2 ?? 'Girmedi'}</TableCell>
                    </TableRow>
                )
              })}
               <TableRow>
                <TableCell colSpan={2} className="font-bold p-1 text-right pr-4 align-top pt-3">Yapıştırma Alanı:</TableCell>
                 {(['exam1', 'exam2', 'perf1', 'perf2'] as GradeField[]).map(gradeType => (
