...
import { Users, Grid, Shuffle, Trash2, Download, GripVertical, AlertTriangle } from 'lucide-react';
...
export function SeatingPlanTab({ students, currentClass, teacherProfile }: SeatingPlanTabProps) {
  ...
  // Clear plan when class or students change
  useEffect(() => {
    setSeatingPlan({});
  }, [currentClass]);
  ...
  const getUnseatedStudents = useCallback(() => {
    const seatedIds = Object.values(seatingPlan).map((s: Student) => s.id);
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2 pt-4 border-t">
              <Button onClick={handleRandomize} disabled={students.length === 0} className="w-full">
                <Shuffle size={18} className="mr-2" /> Dağıt
              </Button>
              <Button onClick={handleClearSeating} variant="destructive" className="w-full">
                <Trash2 size={18} className="mr-2" /> Planı Temizle
              </Button>
              <Button onClick={handleExportWord} disabled={Object.keys(seatingPlan).length === 0} className="w-full">
                <Download size={18} className="mr-2" /> Word Olarak İndir
              </Button>
            </div>
          </CardContent>
        </Card>
          ...
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                <CardTitle className="font-headline flex items-center gap-2">
                  <Users size={20} /> Bekleyen Öğrenciler
                </CardTitle>
            ...
            <CardContent>
              <div
                className="bg-muted/50 p-2 rounded-lg min-h-[100px] max-h-[40vh] overflow-y-auto space-y-2 border border-dashed"
                onDragOver={onDragOver}
                onDrop={onDropList}
              >
                {unseatedStudents.length === 0 && (
                  <div className="text-xs text-muted-foreground text-center py-4">Tüm öğrenciler yerleştirildi.</div>
                )}
                {unseatedStudents.map((student) => (
                  <div
                    ...
                    className="bg-background p-2 rounded-md shadow-sm border flex items-center gap-2 cursor-grab active:cursor-grabbing hover:bg-primary/5 transition-colors"
                  >
                          ...
                          <div
                            ...
                            className={`flex-1 rounded-md flex flex-col items-center justify-center text-center transition-all duration-200 ${
                              student
                                ...
                                : 'bg-white/50 border-2 border-dashed border-amber-400/50 hover:border-amber-500 hover:bg-amber-50/50'
                            }`}
                          >
                            {student ? (
                              <div
                                ...
                                className="relative group/student w-full h-full flex flex-col items-center justify-center cursor-grab active:cursor-grabbing p-1"
                              >
                                ...
                                <div className="absolute -top-1 -right-1 opacity-0 group-hover/student:opacity-100 transition-opacity">
                                  <button
                                    ...
                                    className="bg-red-100 text-red-600 p-0.5 rounded-full hover:bg-red-200"
                                    title="Kaldır"
                                  >
                                    <Trash2 size={10} />
                                  </button>
